...
module vm_nic '.bicep/nested_networkInterface.bicep' = [for (nicConfiguration, index) in nicConfigurations: {
  ...
  params: {
    networkInterfaceName: '${name}${nicConfiguration.nicSuffix}${index}'
  ...
  properties: {
    ...
    networkProfile: {
      networkInterfaces: [for (nicConfiguration, index) in nicConfigurations: {
        ...
        id: az.resourceId('Microsoft.Network/networkInterfaces', '${name}${nicConfiguration.nicSuffix}${index}')
      }]
    }
    diagnosticsProfile: {
      bootDiagnostics: {
...
module vm_customScriptExtension 'extensions/main.bicep' = if (extensionCustomScriptConfig.enabled) {
  ...
  params: {
    ...
    publisher: contains(extensionCustomScriptConfig, 'publisher') ? extensionCustomScriptConfig.publisher : (osType == 'Windows' ? 'Microsoft.Compute' : 'Microsoft.Azure.Extensions')
    type: contains(extensionCustomScriptConfig, 'type') ? extensionCustomScriptConfig.type : (osType == 'Windows' ? 'CustomScriptExtension' : 'CustomScript')
    ...
    settings: {
      ...
      commandToExecute: contains(extensionCustomScriptConfig, 'commandToExecute') ? extensionCustomScriptConfig.commandToExecute : null
    }
...
module vm_diskEncryptionExtension 'extensions/main.bicep' = if (extensionDiskEncryptionConfig.enabled) {
...
module vm_roleAssignments '.bicep/nested_rbac.bicep' = [for (roleAssignment, index) in roleAssignments: {
